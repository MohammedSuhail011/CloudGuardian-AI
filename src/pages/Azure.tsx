...
const serviceIcons: Record<string, React.ComponentType<{ className?: string }>> = {
  ...
  'Key Vault': Database, 'App Services': Activity, NSG: Shield, '': Server,
};

const serviceColors: Record<string, string> = {
  'Virtual Machines': 'text-blue-500', Storage: 'text-green-500', 'Azure AD': 'text-blue-400',
  'Security Center': 'text-purple-500', 'Key Vault': 'text-purple-500', 'App Services': 'text-cyan-500',
  NSG: 'text-red-500', IAM: 'text-yellow-500',
};

const providerColor = '#0089D6';
...
export const Azure = () => {
  ...
  const azureResources = useMemo(() => hasData
    ? resources.filter(r => (r.provider || '').toUpperCase() === 'AZURE')
    : [], [hasData, resources]);
  const azureFindings = useMemo(() => hasAnalysis
    ? analysis.findings.filter(f => f.provider === 'Azure')
  ...
  const serviceGroups = useMemo(() => {
    ...
    for (const res of azureResources) {
      ...
      const count = azureResources.filter(r => r.service === svc).length;
      const svcFindings = azureFindings.filter(f => f.service === svc);
    ...
    return groups;
  }, [azureResources, azureFindings]);

  const severityData = useMemo(() => {
    const severityCounts = { Critical: 0, High: 0, Medium: 0, Low: 0 };
    azureFindings.forEach(f => {
  ...
  const alerts = useMemo(() => azureFindings.map(f => ({
    time: 'now',
  ...
  return (
    ...
    >
      ...
      {!hasData ? (
        <EmptySection message="No dataset loaded — upload a dataset in Threat Dataset Tester to see Azure resources." />
      ) : (
        <>
          <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {serviceGroups.length === 0 ? (
              <div className="col-span-full"><EmptySection message="No Azure resources found in the loaded dataset." /></div>
          ...
          <motion.div variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            >
              ...
              {azureFindings.length > 0 ? (
                ...
                <EmptySection message="No findings for Azure resources." />
              )}
            </motion.div>
          </motion.div>
          ...
          >
            ...
            {alerts.length > 0 ? (
                ...
                {alerts.map((alert, i) => (
                  <motion.div
                    ...
                    className={`flex items-start gap-3 p-3 rounded-lg border cursor-pointer transition-all ${
                      selectedAlert === i ? 'border-[#0089D6]/50 bg-[#0089D6]/5' : 'border-cyber-border hover:border-[#0089D6]/30'
                    }`}
                  >
              ...
              <EmptySection message="No Azure alerts — no findings detected in the loaded dataset." />
            )}
          </motion.div>
            ...
            {azureFindings.length > 0 ? (
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody>
                    {azureFindings.slice(0, 10).map((issue, idx) => (
                      <motion.tr
                        key={issue.id}
                        initial={{ opacity: 0, x: -10 }}
                        whileInView={{ opacity: 1, x: 0 }}
              ...
              <EmptySection message="No misconfigurations detected for Azure resources." />
            )}
          </motion.div>
        </>
      )}
