...
const serviceIcons: Record<string, React.ComponentType<{ className?: string }>> = {
  ...
  CloudTrail: Cloud, GuardDuty: Shield, VPC: Cloud, '': Server,
};

const serviceColors: Record<string, string> = {
  EC2: 'text-orange-500', S3: 'text-green-500', IAM: 'text-blue-500',
...
export const AWS = () => {
  ...
  const awsResources = useMemo(() => hasData
    ? resources.filter(r => (r.provider || '').toUpperCase() === 'AWS')
    : [], [hasData, resources]);
  const awsFindings = useMemo(() => hasAnalysis
    ? analysis.findings.filter(f => f.provider === 'AWS')
  ...
  const serviceGroups = useMemo(() => {
    ...
    for (const res of awsResources) {
      const svc = res.service || 'Unknown';
      if (seenServices.has(svc)) continue;
      seenServices.add(svc);
      const count = awsResources.filter(r => r.service === svc).length;
      const svcFindings = awsFindings.filter(f => f.service === svc);
  ...
  const alerts = useMemo(() => awsFindings.map(f => ({
    time: 'now',
  ...
  return (
    ...
    >
      ...
      {!hasData ? (
        <EmptySection message="No dataset loaded — upload a dataset in Threat Dataset Tester to see AWS resources." />
      ) : (
        <>
          <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {serviceGroups.length === 0 ? (
              <div className="col-span-full"><EmptySection message="No AWS resources found in the loaded dataset." /></div>
          ...
          <motion.div variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            >
              ...
              {awsFindings.length > 0 ? (
                ...
                <EmptySection message="No findings for AWS resources." />
              )}
            </motion.div>
          </motion.div>
          ...
          >
            ...
            {alerts.length > 0 ? (
              ...
              <EmptySection message="No AWS alerts — no findings detected in the loaded dataset." />
            )}
          </motion.div>
            ...
            {awsFindings.length > 0 ? (
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody>
                    {awsFindings.slice(0, 10).map((issue, idx) => (
                      <motion.tr
                        key={issue.id}
                        initial={{ opacity: 0, x: -10 }}
                        whileInView={{ opacity: 1, x: 0 }}
              ...
              <EmptySection message="No misconfigurations detected for AWS resources." />
            )}
          </motion.div>
        </>
      )}
